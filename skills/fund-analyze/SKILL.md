...
description: >-
  ...
  前瞻收益分布(1/3/6月) + 深度解读 + 离场信号，并生成一份**可视化 HTML 报告**（K线/净值+回撤+持仓图）在浏览器打开，最后把判断存档以便复盘。
...
- 首次使用需把引擎装成全局命令（一次性）：在仓库跑 `bash skills/fund-analyze/scripts/setup.sh`（即 `uv tool install`）；装好后任何目录都能直接 `quantfox ...`。
...
7. **打四维评分卡 + 结论**：严格按 `quantfox/prompts/analysis_framework.md`——趋势动量/稳定性/估值/基本面质量各 0-100，综合出 Verdict + 信心分 + **离场信号(Kill criteria)**。data_quality 缺失要下调信心并说明。
   ...
   - **回测背书门槛**：出"买/回避"前先跑 `quantfox backtest <标的> --rule combo`。基线 **edge≤0 或 net≤0 → 默认降回"观望"**，除非有具体强力的非机械证据并显式说明为何敢推翻基线。宗旨：极少出手、每次出手都有历史基线撑腰。
   - **反方验证**：下结论前先当"魔鬼代言人"专门反驳自己，列最强反面证据；驳不倒才保留，否则降档/降信心。把这轮攻防写进解读。宁可弃权，不硬猜。
8. **生成可视化报告并打开**（闭环关键，必做）：
...
9. **存档预测**（复盘地基，别漏）：
   ...
   quantfox evidence <标的> --format json > /tmp/ev.json
   quantfox log-signal --symbol <代码> --type <otc_fund|gold> \
   ...
   ```
10. **对话里也给结论**：结论先行（信号+信心）+ 四维小结 + **前瞻中位/正收益概率一句话** + 一句风险 + 报告已打开提示 + 免责。
...
- **前瞻只给分布不给点数字**：`forecast` 看中位不看均值；估值分位 >0.85 以 `from_similar_valuation` 为准；明说是历史推断、样本偏牛市、非保证。
