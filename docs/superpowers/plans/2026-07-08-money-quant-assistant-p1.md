...
**Goal:** 构建一个 Python CLI（`money`）+ Claude Code Skill（`fund-analyze`），对场外基金与黄金取数、算指标、收集舆情、组装"证据卡"，由 Claude 综合推理出可解释信号，并把每次预测存档以便复盘评估。
...
- 包管理：uv；项目名 `money-quant`，导入名 `money`。
...
**Interfaces:**
- Produces: 可用的 `uv run pytest`；导入包 `money`（暴露 `__version__`）。
...
**Files:**
- Create: `money/prompts/analysis_framework.md`
- Create: `money/prompts/__init__.py`
- Test: `tests/test_framework.py`

**Interfaces:**
- Produces: `money/prompts/framework_path() -> Path` 与 `framework_version() -> str`（读取文件首行 `<!-- version: N -->`）。
...
`money/prompts/analysis_framework.md`:
```markdown
<!-- version: 1 -->
# 分析框架（判断层唯一真理源）
...
```bash
git add money/prompts tests/test_framework.py
...
- Produces: typer `app`，命令：
  - `money evidence <query> [--format json|markdown]`：解析→取价→取新闻→查战绩→组装证据卡→打印。
  - `money fetch <query>` / `money indicators <query>` / `money news <query>`：调试用。
  - `money log-signal ...` / `money outcomes` / `money review [query] [--all] [--since V]`。
...
**Interfaces:**
- Produces: 一个 SKILL.md，指导 Claude：跑 `money review` 看战绩 → 跑 `money evidence <q> --format json` 取证据卡 → 用 WebSearch 补最新舆情 → 按 `analysis_framework.md` 推理 → 输出信号 → 跑 `money log-signal` 存档。
...
2. 看历史战绩（校准自己）：
   `money review <标的>`
3. 取证据卡：
   `money evidence <标的> --format json`
   读取其中 price/indicators/percentile/news/track_record/data_quality。
4. 补最新舆情：用 WebSearch/WebFetch 搜该标的近期新闻、公告、讨论，鉴别真伪与来源可信度。
5. 按判断框架推理：读取并遵守 `money/prompts/analysis_framework.md` 的信号档位与输出要求。
6. 产出结论：信号 + 置信度 + 采信/忽略了哪些舆情及原因 + 量化支撑 + 风险 + 基于战绩的校准说明。
7. 存档本次预测（用于日后复盘）：
   `money log-signal --symbol <代码> --type <otc_fund|gold> --signal <档位> --signal-numeric <2..-2> --confidence <0-1> --price-ref <证据卡最新价> --ts <今天YYYY-MM-DD> --horizons 5,20,60 --rationale "<一句话理由>"`
...
Run:
```bash
uv run money evidence gold --format markdown
uv run money evidence 501018 --format json | head -40
```
Expected: 打印出证据卡；若 akshare 接口报错，回 Task 3/6 按实际列名修正后重跑。把实际输出贴进完成报告。

- [ ] **Step 3: 写 README（安装 + 用法 + 免责声明）**

README 包含：`uv sync` 安装、`money evidence` 用法、Skill 用法、以及**明确免责声明**（非投资建议、不保证盈利、决策自负）。
